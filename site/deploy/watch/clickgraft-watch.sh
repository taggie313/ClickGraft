...
#
#   clickgraft-watch.sh                  # the service: tail the log, notify
  ...
  while IFS='|' read -r kind prefix ref ua; do
    ...
    case "$kind" in
      ...
      asset)
        ...
        rm -f "$pend"
        [ "$(( $(date +%s) - pend_at ))" -gt "$CONFIRM_WINDOW" ] && continue
        kind=view
        ;;
    esac
    ...
    now=$(date +%s)
    last=0
    [ -f "$STATE/$key" ] && last=$(cat "$STATE/$key" 2>/dev/null || echo 0)
    [ "$((now - last))" -lt "$quiet" ] && continue
    printf '%s\n' "$now" > "$STATE/$key"
...
echo "watching $LOG -> $NTFY_URL/$NTFY_TOPIC"

# -n0: only what arrives from now on. -F: survives the log being rotated or
# truncated out from under us.
tail -n0 -F "$LOG" 2>/dev/null | events | notify
